...
try:
    ...
    app.logger.warning("⚠ Could not connect to MongoDB: %s", e)
        ...
        if model_path and load_model is not None:
            ...
            if json_path:
                try:
                    ...
                    idx_to_class = {}
                    logger.warning(f"⚠ Failed to load or parse class_indices.json: {e}")
            else:
                idx_to_class = {}
                logger.info("ℹ No class_indices.json found; predictions will return indices only.")
        else:
            disease_model = None
            idx_to_class = {}
            logger.info("⚠ Disease model not found or TensorFlow not installed.")
    except Exception as e:
        disease_model = None
        idx_to_class = {}
...
# Around line 339 in the ask_gemini_once function
